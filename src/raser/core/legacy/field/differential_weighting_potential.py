...
from raser.core.field import devsim_draw
from raser.core.field.assets import resolve_field_pickle
from raser.supports.output import create_path

def main(v, electrode_name, field_directory):
    voltage = float(v)
    directory = Path(field_directory)

    potential_file = resolve_field_pickle(directory, "Potential", voltage)
    added_potential_file = resolve_field_pickle(
        directory / "weightingfield" / electrode_name,
        "Potential",
        voltage,
    )
    ...
    path = directory / "weightingfield" / electrode_name
    create_path(path)
