    ...
    def __init__(
        ...
        *,
        ...
        if batch == -1 and (
            ...
            or my_g4.geant4_model == "charge_collection"
            or my_g4.geant4_model == "bmos"
            or my_g4.geant4_model == "beta_source"
        ):
            total_step = 0
            particle_number = 0
            for (
                p_step
            ) in my_g4.p_steps_current:  # selecting particle with long enough track
                if len(p_step) > 1:
                    particle_number = 1 + particle_number
                    total_step = len(p_step) + total_step
            for j in range(len(my_g4.p_steps_current)):
                if len(my_g4.p_steps_current[j]) > (
                    (total_step / particle_number) * 0.5
                ):
        ...
        for i in range(self.layer):
            ...
            for k in range(len(my_g4.devicenames[j])):
                ...
                if name in my_g4.devicenames[j][k]:
                    ...
                    tp[0] = (
                        my_g4.localpositions[j][k][0] + (px - 0.5) * self.pixelsize_x
                    )
                    tp[1] = (
                        my_g4.localpositions[j][k][1] + (py - 0.5) * self.pixelsize_y
                    )
