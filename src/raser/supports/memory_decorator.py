...
def memory_decorator(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print(
            f"Memory usage before calling {func.__name__}: RSS = {rss_before:.2f} MB, VMS = {vms_before:.2f} MB"
        )
        ...
        print(
            f"Memory usage after calling {func.__name__}: RSS = {rss_after:.2f} MB, VMS = {vms_after:.2f} MB"
        )
        print(
            f"Memory increase: RSS = {rss_after - rss_before:.2f} MB, VMS = {vms_after - vms_before:.2f} MB"
        )
